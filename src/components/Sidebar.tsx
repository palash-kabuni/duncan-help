...
import { Brain, LayoutDashboard, Plug, Settings, LogOut, UserCircle, ShoppingCart, Users, Activity, Bug } from "lucide-react";
...
const Sidebar = () => {
  ...
  return (
    <aside className="fixed left-0 top-0 z-40 flex h-screen w-64 flex-col border-r border-border bg-sidebar">
      ...
      {/* User & Settings */}
      <div className="relative border-t border-border px-4 py-4 space-y-2">
        {user && (
        ...
        <button
          ...
          className={cn(
            "flex items-center gap-2 rounded-md px-2 py-1.5 text-xs font-medium transition-all duration-150 w-full",
        ...
        {showMenu && (
          <>
            ...
            <div className="absolute bottom-full left-3 mb-2 z-50 w-[calc(100%-1.5rem)] rounded-lg border border-border bg-card shadow-lg py-1">
              {settingsMenuItems.map((item) => (
                <button
                  key={item.to}
                  onClick={() => { navigate(item.to); setShowMenu(false); }}
                  className="flex items-center gap-2.5 w-full px-3 py-2 text-xs font-medium text-foreground/80 hover:bg-sidebar-accent hover:text-foreground transition-colors"
                >
