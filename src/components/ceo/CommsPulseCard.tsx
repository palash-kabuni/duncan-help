...
export default function CommsPulseCard({ emailPulse, slackPulse, hubspotSignal, githubSignal }: Props) {
  ...
  const showExternalSignals = hubspotSignal?.connected === true && githubSignal?.connected === true;
  ...
  const hasAnyComms = !!(emailPulse?.per_mailbox || slackPulse || showExternalSignals);
  if (!hasAnyComms) {
    return (
