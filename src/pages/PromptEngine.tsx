...
const PromptEngine = () => {
  ...
  // Sanitize copy to strip dark-theme styles but preserve readable formatting
  useEffect(() => {
    const handler = (e: ClipboardEvent) => {
      ...
      const sanitize = (node: Node) => {
        if (node.nodeType === Node.ELEMENT_NODE) {
          ...
          // Remove all existing styles, classes, data attrs
          ...
          // Apply clean document styles per element type
          if (["h1", "h2", "h3", "h4", "h5", "h6"].includes(tag)) {
            const sizes: Record<string, string> = { h1: "22px", h2: "18px", h3: "16px" };
            el.style.color = "#111";
            el.style.fontSize = sizes[tag] || "14px";
